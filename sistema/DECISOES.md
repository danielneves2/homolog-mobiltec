...
| D472 | Unificação do modal de exibição de informações e centralização de observações funcionais e gerais com download de anexos | Elimina a discrepância de modais entre o Painel do Parceiro e o fluxo de Validação de Certificados (`ValidarCertificados.tsx`), consolidando um único componente com abas segmentadas ('Testes Executados' e 'Observações (N)'). A contagem e a listagem de observações passam a computar todas as anotações funcionais registradas (seja em `observacao`, `justificativaTexto` ou no objeto `justificativa`), sem mascaramento em tooltip `?`, além de todas as observações gerais da homologação. Implementa botão de download direto e lightbox para prints, logs e arquivos .zip anexados em ambas as seções. |
| D473 | Padronização visual e estrutural do modal 'Exibir Informações' baseado no layout de Validar Certificado | Adota o layout linear da ficha de homologação (cabeçalho, ficha técnica, tabelas de grupos e bloco de observações gerais) como o padrão visual único em 'Validar Certificado → Exibir informações' e 'Painel do Parceiro → Exibir informações'. Cada linha da tabela de funcionalidades expõe diretamente a observação registrada pelo parceiro e a justificativa técnica com cards dedicados (sem ocultar em tooltip `?`), além de botões para download e ampliação de anexos (.zip, logs e prints). O componente passa a ser estritamente compartilhado entre os dois fluxos sem duplicar estruturas. |
| D474 | Separação da Matriz de Testes e Observações em abas no modal Exibir Informações | Limpa a tabela da Matriz de Testes removendo cards de observações/justificativas e chips de anexos das linhas da tabela, mantendo-a estritamente tabular (Item | Ação | Status). Insere barra de navegação com abas segmentadas ('Matriz de Testes' e 'Observações (N)') ao lado de 'Resultado da homologação'. Na aba 'Observações', organiza separadamente em seções dedicadas as 'Observações por funcionalidade' (com badge de status, texto, justificativa técnica e botões de download/ampliação de logs/prints/zips) e as 'Observações gerais' da homologação, mantendo layout 100% compartilhado e idêntico entre Validar Certificado e Painel do Parceiro. |
| D475 | Reabertura de homologações de parceiro migra para EM_REVISAO e expõe botão Reabrir em Validar Certificado | No endpoint `/homologacoes/:id/reabrir`, quando o registro pertencer a um parceiro (`papel === 'PARCEIRO'` ou empresa externa), o status transiciona para `EM_REVISAO` (em vez de `RASCUNHO`), garantindo que o dispositivo continue rastreável na aba 'Em revisão' de `ValidarCertificados.tsx`, notifique o parceiro com o card de revisão técnica e libere a edição de testes na matriz com o botão 'Reenviar para Validação' (`EM_REVISAO → AGUARDANDO_ANALISE`). Além disso, adiciona ação direta de 'Reabrir' nos cards de homologações da aba 'Aprovados' em `ValidarCertificados.tsx`, permitindo correções ágeis de aprovações acidentais sem sair do fluxo de validação. |
| D476 | Regra de certificado por ambiente, confidencialidade de Apoio Adicional e padronização técnica Mobiltec | Define a exibição dinâmica do certificado por ambiente: no Painel do Parceiro (ambiente exclusivo), o Apoio Adicional é exibido caso tenha sido informado (nome e empresa) e omitido caso contrário; no Painel Público da Mobiltec (catálogo e visualização geral), o Apoio Adicional é estritamente omitido para preservar a confidencialidade entre parceiros. Além disso, padroniza as assinaturas oficiais: Responsável Técnico passa a ser dinamicamente o Admin da Mobiltec que aprovou a homologação (`HistoricoStatus` com `statusNovo === 'APROVADO'`), eliminando nomes de técnicos parceiros (como Matheus) no padrão oficial; Gerente de Validação é fixado em Rafael Cordeiro em todos os certificados e fichas técnicas (`FichaHomologacao.tsx`). As rotas de preview e download PDF (`/certificado/preview` e `/certificado/pdf`) passam a suportar o parâmetro `ambiente`, impedindo acesso de terceiros com validação de autoria no backend sem alterar o registro original do banco. |
